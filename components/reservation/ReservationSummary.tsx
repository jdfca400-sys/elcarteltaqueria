...
import { Users, Calendar, Clock, MapPin, RefreshCcw, Check, PartyPopper } from "lucide-react"
    ...
    const JourneyStep = ({
        ...
        return (
            <motion.div
                layout
                className={cn(
                    "relative flex items-center gap-2 md:gap-3 px-3 py-2 md:px-4 md:py-3 rounded-xl border transition-all duration-300 w-full overflow-hidden",
                    isFilled
            ...
            >
                <div className={cn(
                    "flex items-center justify-center w-6 h-6 md:w-8 md:h-8 rounded-full transition-colors shrink-0",
                    isFilled ? "bg-[#00ff88] text-black" : "bg-white/10 text-gray-400"
                )}>
                    {isFilled ? <Check className="w-3 h-3 md:w-4 md:h-4" /> : <Icon className="w-3 h-3 md:w-4 md:h-4" />}
                </div>
